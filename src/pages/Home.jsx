...
const OrderInfo = ({ order }) => {
  ...
  return (
    <Paper elevation={3} sx={{ p: 3, mb: 4, borderRadius: 2 }}>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
        ...
        <Chip 
          ...
          color={
            ...
            order.orderStatus === 'PREPARING' ? 'warning' : 'primary'
          }
        />
      </Box>
      
      <Box sx={{ mb: 2 }}>
        <Typography variant="subtitle1" fontWeight="bold">{order.storeName}</Typography>
        <Typography variant="body2" color="text.secondary">{order.storeAddress}</Typography>
        <Chip 
          label={order.pickupType} 
          variant="outlined" 
          size="small" 
          sx={{ mt: 1 }}
        />
      ...
      <Typography variant="subtitle2" fontWeight="bold" gutterBottom>주문 내역</Typography>
      <List dense>
