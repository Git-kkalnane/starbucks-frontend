...
const MenuItem = ({ item, className }) => {
    return (
        <div className={`flex py-2 border-b border-gray-100 ${className}`}>
            <div className="w-24 h-24 rounded-full bg-gray-100 mr-4 flex-shrink-0"></div>
...
const MenuList = ({ activeTab, drinkItems, foodItems, className }) => {
    return (
        <section className={`px-1 ${className}`}>
            <div className="space-y-4 overflow-y-auto h-[calc(100vh-220px)]">
                ...
                {activeTab === '푸드' && foodItems.map((item) => <MenuItem key={item.id} item={item} />)}
            </div>
        </section>
    );
};
